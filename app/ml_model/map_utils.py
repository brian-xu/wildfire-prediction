...
class MapUtils:
    def __init__(self, area: int):
        ...
        # Generated by rasterio from the LANDFIRE GeoTiffs.
    ...
    def rr_indices(self, lat: float, lon: float) -> [int, int]:
        """
        The Rapid Refresh dataset roughly projects the geometry of the United States to a 13km grid.
        This function calculates the correct indices for the given latitude and longitude.
        """
    ...
    def reproj_viirs(self, viirs_data: str) -> None:
        """
        Given a string containing VIIRS data, creates and stores a geopandas GeoDataFrame
        containing the latitude, longitude, and projected coordinates of each point.
        """
    ...
    def read_tiff(self, tiff: rasterio.DatasetReader, lat: int, lon: int) -> np.array:
        """
        Given a LANDFIRE GeoTiff opened in rasterio and a specific latitude and longitude,
        return a bounding window 11.25km in size centered on that latitude and longitude.
        """
    ...
    def resample_landfire(self, lf_data: np.array) -> np.array:
        """
        Given a NumPy array corresponding to LANDFIRE features, use an average
        pooling operation to reshape it to an array of a specific size.
        """
